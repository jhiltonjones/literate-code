...
def calculate_bending_angle(realimage):
    ...
    # Apply Gaussian Blur to reduce noise while keeping edges
    ...
    # Find contours from the cleaned binary mask
    contours, _ = cv2.findContours(binary_mask_cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Sort contours by area and keep the largest one (assuming it's the rod)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)
    from scipy.interpolate import CubicSpline


    # Improved morphological operations to remove more noise
    kernel_large = np.ones((5, 5), np.uint8)  # Increase kernel size
    ...
    contours, _ = cv2.findContours(binary_mask_cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)  # Keep largest contour
    ...
    if len(contours) > 0:
        ...
        # Sort points for proper curve fitting
        sort_idx = np.argsort(x_vals)
        x_vals = x_vals[sort_idx]
        y_vals = y_vals[sort_idx]

        # Fit a quartic polynomial (degree 4) for better curvature estimation
    # Use a cubic polynomial instead of quartic to prevent overfitting at endpoints
    poly_coeffs = np.polyfit(x_vals, y_vals, 3)  # Lower-degree fit
    cubic_fit = np.poly1d(poly_coeffs)

    # Restrict x values to the valid range of the contour
    x_fit_range = np.linspace(x_vals.min(), x_vals.max(), num=100)

    # Select multiple key points for tangent estimation
    key_indices = np.linspace(0, len(x_vals) - 1, num=5, dtype=int)
    key_x = x_vals[key_indices]
    key_y = cubic_fit(key_x)

    # Compute slopes at key points
    slopes = [3 * poly_coeffs[0] * x**2 + 2 * poly_coeffs[1] * x + poly_coeffs[2] for x in key_x]

    # Compute pairwise bending angles
    angles = [np.arctan(abs((slopes[i+1] - slopes[i]) / (1 + slopes[i] * slopes[i+1]))) for i in range(len(slopes) - 1)]

    # Compute the final bending angle as the average of segment-wise angles
    bending_angle_deg = np.degrees(np.mean(angles))

    # # Plot the improved results with restricted fit range
